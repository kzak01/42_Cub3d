...
void	step_side(t_math *math, t_game *game)
{
	...
	{
		math->step_x = -1;
		math->side_dist_x = (game->player.pos_x - math->map_x) * math->delta_dist_x;
	}
	else
	{
		math->step_x = 1;
		math->side_dist_x = (math->map_x + 1.0 - game->player.pos_x) * math->delta_dist_x;
	}
	if(math->ray_dir_y < 0)
	{
		math->step_y = -1;
		math->side_dist_y = (game->player.pos_y - math->map_y) * math->delta_dist_y;
	}
	else
	{
		math->step_y = 1;
		math->side_dist_y = (math->map_y + 1.0 - game->player.pos_y) * math->delta_dist_y;
	}
}
...
{
	if (math->side == 0)
		math->wall_x = game->player.pos_y + math->perp_wall_dist * math->ray_dir_y;
	else
		math->wall_x = game->player.pos_x + math->perp_wall_dist * math->ray_dir_x;
}
...
void	calculate_pixel(t_math *math, t_game *game)
{
	...
	math->tex_pos = (math->draw_start  - W_HEIGHT / 2 + math->line_h / 2) * math->step;
}
